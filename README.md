# AmongUsModManager
Among UsのModを選ぶだけで自動で導入を行ってくれるアプリです。<br>
（.zipファイルをご用意いただくことでリストにないmodでもインストールができます）

ホームページは[**こちら**](https://amongusmodmanager.web.app/)<br>
証明書は[こちら](https://github.com/Tabasco1410/AmongUsModManager/releases/download/1.4.3/AmongUsModManager_1.4.3.0_x86_x64_arm64.cer)

## [Discord](https://discord.gg/nFhkYmf9At)<br>

## 利用規約
LICENSEをご覧ください。<br>
Among Us_ModManeger

このソフトウェアは GNU General Public License v3.0 に基づいて公開されています。

・自由に使用、再配布、改変できます。<br>
・ただし、改変して再配布する場合は、同じGPLv3ライセンスのもとで公開し、ソースコードを公開する必要があります。<br>
　詳しくは開発者までお問い合わせください。<br>
・本ソフトウェアの使用によって発生したいかなる損害に対しても、開発者は責任を負いません。<br>

詳しいライセンスの内容は、同梱の LICENSE ファイル、または下記を参照してください。<br>
https://www.gnu.org/licenses/gpl-3.0.txt
<br>
Copyright (c) 2025 Tabasco1410</br>
※本アプリはAmong Usの開発元であるInnersloth,各種Mod関係者様とは一切関係なく、非公式のアプリです。

## Among UsのModを導入する皆さんへ
**公開部屋でのmod部屋への誘導は禁止です。**<br>
バニラ公開部屋でmod部屋のコードを貼ったり、コードを貼りに来る人を待機する部屋を作ることは禁止されています。<br>
絶対に行わないでください。<br>
これらの行為をした場合、MOD共通のブラックリストに登録されることがあります。<br>
もしこのような行為を発見した場合は、TownOfHost-KのDiscordにて報告をお願いします。<br>
（配信やDiscordなどで非公開ルームのコードを共有して遊ぶのには問題ありません。）


## インストールガイド

ご利用の環境に合わせて、以下のいずれかのファイルをダウンロードしてください。

### 1. 標準インストーラー（推奨）
**ファイル名: AmongUsModManager_Setup-1.4.4.0.exe**
* **手順**: 
    1. ファイルをダウンロードして実行します。
    2. 画面の指示に従ってインストールを完了させてください。
* **注意**: 新しいバージョンがリリースされた際は、再度最新のインストーラーをダウンロードして上書きインストールする必要があります。新しいバージョンがリリースされた際にはアプリが自動でダウンロードして実行します。

---

### 2. 単一実行ファイル版（パッチ・ポータブル用）
**ファイル名: AmongUsModManager-1.4.4-x64.exe (または x86 / arm64)**
* **手順**: 
    1. 自身のPCのアーキテクチャに合ったファイルをダウンロードします。
    2. 任意の場所に配置して直接実行してください。
※アップデートの際は新しいファイルをダウンロードしなおす必要があります。ご注意ください。
---
### 3. 自動更新パッケージ（上級者向け）
#### 手順1：証明書のインポート
* **ファイル名: AmongUsModManager.cer**
* **必須作業**: 最初にこのファイルをダウンロードして開きます。
* **インポート先**: 証明書ストアの選択画面で、必ず **「信頼されたルート証明機関」** を選択してインストールしてください。この作業を行わないと、次のアプリ本体をインストールできません。

#### 手順2：アプリのインストール
* **ファイル名: AmongUsModManager.appinstaller**
* **特徴**: 一度インストールすると、以降はアップデートがあるたびにアプリが自動で更新されます。
* **補足**: 手動で特定のバージョンを維持したい場合は、**AmongUsModManager_1.4.4.0_x86_x64_arm64.msixbundle** を使用してインストールすることも可能です。
